...
Trie.prototype.insert = function (sub, j) {
  let cur = this.root;
  for (let i = 0; i < sub.length; i++) {
     let index = (sub[i] === '\'' ? 26 : sub[i].toLowerCase().charCodeAt(0) - 97);
     if (cur.children[index] === undefined) {
...
var build = function (list) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    for (let j = 0; j < temp.length; j++) {
      ...
      for (let k = 0; k < len; k++) {
        for (let step = k + 1; step <= len; step++) {
          let sub = temp[j].substring(k, step);
          root.insert(sub, i);
        }
      }
    }
...
function nonPrefixSearch (str, list) {
  let res = [];
  let node = build(list);
  node = node.root;
 
  for (let i = 0; i < str.length; i++) {
    let idx = (str[i] === '\'' ? 26 : str[i].toLowerCase().charCodeAt(0) - 97);
    if (node.children[idx] === undefined) {
        return res;
    }
    node = node.children[idx];
  }
  for (let key of node.words) {
    res.push(list[key].business_name);
  }
  return res;
}

nonPrefixSearch('Bur', dictionaries)
